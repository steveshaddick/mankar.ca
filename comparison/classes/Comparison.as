	...
	*/
	public class Comparison extends MovieClip 
	{
		...
		public function showComparison(_variables:Array, _ref:String, _isMetric:Boolean):void 
		{
			...
			if (this.objComparison.dollarsign == "1")
			{
				txtReference1.appendText(_variables['mankar'][_ref].toFixed(2));
			} else {
			...
			if (this.objComparison.dollarsign == "1")
			{
				txtReference2.appendText(_variables['other'][_ref].toFixed(2));
			} else {
			...
			} else if (_variables['mankar'][_ref] < _variables['other'][_ref]){
			...
			while (i < str.length) {
				...
				if (isVar) {
					...
					if (str.charAt(i - 3) == "$") {
						...
						tmpdollar = _variables[str.substring(i, str.indexOf("#", i))][str.substring(str.indexOf("#", i) + 1, str.indexOf("&&", i))].toFixed(2);
					} else {
						tmpdollar = _variables[str.substring(i, str.indexOf("#", i))][str.substring(str.indexOf("#", i) + 1, str.indexOf("&&", i))];
					}
					
					parsed.push(tmpdollar);
					i = str.indexOf("&&", i) + 2;
					start = i;
					end = i;
					isVar = false;
