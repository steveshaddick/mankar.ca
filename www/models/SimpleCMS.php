...
class SimpleCMS {
	...
	public function getTradeshow($insert = false) {
		if ($insert) {
			return $this->getFields('tradeshows');
		} else {
			return $this->mySQL->getSingleRow("SELECT * FROM tradeshows WHERE show_id = $this->actionData");
		}
	}

	public function saveTradeshow($action) {
	...
	public function deleteTradeshow() {
		$this->mySQL->sendQuery("DELETE FROM tradeshows WHERE show_id = $this->actionData");
