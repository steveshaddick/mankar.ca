		...
		foreach( $file as $this_file ) {
			if( $this_file != "." && $this_file != ".." ) {
				if( is_dir("$directory/$this_file") ) {
					...
					//nick is exploding the directory
					$urlToExplode = $directory;
					$delimiter = "material/";
					$explodedUrl = explode($delimiter,$urlToExplode);
					$containing_dir = end($explodedUrl); 
			
					$link = str_replace("[link]", "$containing_dir/" . urlencode($this_file), $return_link);
